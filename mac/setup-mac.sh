...
function install_homebrew() {
    ...
    if hash brew 2>/dev/null; then
        ...
        if /usr/bin/ruby -e "$(curl -fsSL ${BREW_INSTALLER})"; then
...
function clone_dotfiles_repo() {
    ...
    if test -e $DOTFILES_REPO; then
        ...
        if git clone "$GITHUB_REPO" $DOTFILES_REPO && \
           git -C $DOTFILES_REPO remote set-url origin $GIT_REPO; then
...
function setup_vim() {
    ...
    if test -e ~/.vim/bundle/Vundle.vim; then
        ...
        if git clone "$VUNDLE_URL" ~/.vim/bundle/Vundle.vim; then
...
function setup_tmux() {
    ...
    if test -e ~/.tmux/plugins/tpm; then
        ...
        if git clone "$TPM_URL" ~/.tmux/plugins/tpm; then
...
function update_hosts_file() {
    ...
    if sudo wget --quiet --output-document="${downloaded_hosts_file_path}" "$HOSTS_URL" ; then
